...
public class DiscordNotifs extends Module
{
    ...
    {
        super(FoundationAddon.CATEGORY, "discord-notifs", "Sends notifications to a discord webhook.");
    }
        ...
        new Thread(() -> {
            try {
                ...
                if (responseCode == 204 || responseCode == 200) {
                    FoundationAddon.LOG.info("Successfully sent message to webhook!");
                } else {
                    FoundationAddon.LOG.warn("Webhook response code: " + responseCode);
                    FoundationAddon.LOG.warn("Failed to send to Webhook");
                }

                conn.disconnect();
            } catch (Exception e) {
                FoundationAddon.LOG.warn("Failed to send to webhook: " + e.getMessage());
            }
        }).start();
    }
